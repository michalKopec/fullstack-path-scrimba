    ...
    menuArray.forEach(function(item){
        menuHTML += `
            <article class="menu-item">
                <img class="menu-item__picture" src="images/${item.name}-item.png" alt="${item.alt}">
                <ul class="menu-item__details">
                    <li class="menu-item__name">${item.name}</li>
                    <li class="menu-item__ingredients">${item.ingredients}</li>
                    <li class="menu-item__price">$${item.price}</li>
                </ul>
                <button class="menu-item__add-btn" data-add="${item.id}">+</button>
            </article>
        ` 
    }) 
    return menuHTML; 
...
function getOrderHTML(itemNumber) {
    ...
    let orderList = `
        <div class="order-summary__item">
            <dt class="order-summary__item-name">${orderItem.name}</dt>
            <button class="order-summary__remove-btn" type="button" aria-label="Remove ${orderItem.name}" data-remove="${orderItem.id}">remove</button>
            <dd class="order-summary__item-price">$${orderItem.price}</dd>
        </div>
    `
...
// Find which item to add to order 
